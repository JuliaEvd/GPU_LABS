...
size_t localSize_cpu = 256;
    ...
    const char* saxpy_g = "__kernel void s_gpu(\n"
		"const int n, float a,__global float * x, const int incx, __global float * y, const int incy) {\n"
        "int i = get_global_id(0);\n"
        "if(i<n) \n"
        "y[i*incy] = y[i*incy]+a*x[i*incx]; \n"
        "}\n";
	const char* daxpy_g = "__kernel void d_gpu(\n"
		"const int n, double a,__global double * x, const int incx, __global double * y, const int incy) {\n"
		"int i = get_global_id(0);\n"
		"if(i<n) \n"
		"y[i*incy] = y[i*incy]+a*x[i*incx]; \n"
		"}\n";
	...
	size_t srclen[] = { strlen(saxpy_g) };

	cl_program program = clCreateProgramWithSource(context, 1, &saxpy_g, srclen, &error);
    if (error != CL_SUCCESS) {
	...
	cl_kernel kernel = clCreateKernel(program, "s_gpu", &error);
    if (error != CL_SUCCESS) {
	...
	cl_mem y = clCreateBuffer(
		...
		NULL,
		NULL);
	clEnqueueWriteBuffer(queue, x, CL_TRUE, 0, sizeof(float) * SIZE, data_x, 0, NULL, NULL);
	error = clEnqueueWriteBuffer(queue, y, CL_TRUE, 0, sizeof(float) * SIZE, data_y,0,NULL, NULL);
	...
	// -------------------------- N?aaiaiea ?acoeuoaoia CPU & GPU float --------------------------
	...
	// -------------------------- N?aaiaiea ?acoeuoaoia CPU & OMP float --------------------------
	...
	// -------------------------- N?AAIAIEA ?ACOEUOAOIA GPU VS CPU DOUBLE --------------------------
	error = 0;
	...
	//device_d = nullptr;
	cl_command_queue queue_d = clCreateCommandQueue( context_d,
		device_d,
	...
	size_t srclen_d[] = { strlen(daxpy_g) };
	...
	// -------------------------- N?aaiaiea ?acoeuoaoia (CPU & GPU) double --------------------------
	...
	// -------------------------- N?aaiaiea ?acoeuoaoia (CPU & OMP) double --------------------------
	...
	cl_device_id device_fcpu = nullptr;
	...
	cl_command_queue queue_fcpu = clCreateCommandQueue( context_fcpu, device_fcpu, CL_QUEUE_PROFILING_ENABLE, &error);

	if (error != CL_SUCCESS) {
		std::cout << "1 Create command queue with properties failed: " << error << std::endl;
	}

	size_t srclen_fcpu[] = { strlen(saxpy_g) };
	...
	cl_device_id device_dcpu = nullptr;
	...
	if (error != CL_SUCCESS) {
		std::cout << "2 Create command queue with properties failed: " << error << std::endl;
	}

	size_t srclen_dcpu[] = { strlen(daxpy_g) };
